...
Бот підтримує отримання новин за категоріями, автоматичну генерацію описів новин, підписку на регулярні оновлення та можливість відписки від них. Передбачено обробку помилок і валідацію даних, включаючи перевірку URL та обробку зображень.
...
Збір новин реалізовано через модуль news_scraper.py, який парсить RSS-фіди та веб-сайти, обробляє HTML, перевіряє коректність посилань та завантажує зображення. Модуль ai_generate.py дозволяє обробляти текст новин, створювати стислий опис або узагальнення. Модуль handlers.py містить обробники команд Telegram, таких як /start, /help, /news, /category, /subscribe, /unsubscribe та /about. Головний файл run.py ініціалізує бота та підключає всі модулі.

Бот реалізований на Python з використанням бібліотек python-telegram-bot для взаємодії з Telegram API, requests для HTTP-запитів, BeautifulSoup4 для парсингу HTML та Flask для підтримки keep-alive механізму. Параметри підключення до бази даних або зовнішніх API зберігаються у файлі config.py та можуть бути задані через файл .env.

Для запуску проєкту необхідно клонувати репозиторій, встановити залежності, створити файл .env із токеном Telegram бота та ключем API для AI (якщо використовується), а потім запустити головний файл run.py. Бот буде доступний у Telegram за зазначеним username.
